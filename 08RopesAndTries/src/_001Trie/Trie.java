...
public class Trie<Value> {
    ...
    private Node insert(Node node, String key, Value value, int v) {
        ...
        Map<Character, Node> children = node.getNext();
        char childrenKey = key.charAt(v);

        Node newNode = null;

        if (children.containsKey(childrenKey)) {
            newNode = children.get(childrenKey);
        }

        children.put(childrenKey,insert(newNode,
                key, value, v + 1));
